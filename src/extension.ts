...
export function activate(context: vscode.ExtensionContext) {
    ...
    let disposable = vscode.commands.registerCommand('skylight.helloWorld', () => {
        // The code you place here will be executed every time your command is executed
        // Display a message box to the user
        const sensor = LightSensor.getDefault();
        vscode.window.showInformationMessage(sensor.getCurrentReading().illuminanceInLux.toString());
        // vscode.window.showInformationMessage(Object.keys(LightSensor).join(", "));
    });
